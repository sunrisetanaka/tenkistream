...
def ml(df,nowdf):
    ...
    history = LossHistory()
    #学習だけコメントアウトで切っておく
    #hist=model.fit({"input_a":X1_train,"input_b":X2_train,"input_c":X3_train,"input_d":X4_train},{"output1":y1_train,"output2":y2_train,"output3":y3_train},epochs=n_epochs,validation_data=({"input_a":X1_test,"input_b":X2_test,"input_c":X3_test,"input_d":X4_test},{"output1":y1_test,"output2":y2_test,"output3":y3_test}),verbose=1,batch_size=batch_size,callbacks=[history])
    #代わりにロード
    model=load_model("tenki.h5")
